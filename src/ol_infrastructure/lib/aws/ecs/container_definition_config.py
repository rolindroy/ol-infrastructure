...
class LinuxParameters(BaseModel):
    ...
    tmpfs: Optional[List[Tmpf]] = Field(
        None,
        description=(
...
class DockerLabels(BaseModel):
    """
    A key/value map of labels to add to the container.  This parameter maps to Labels in
    the https://docs.docker.com/engine/api/v1.35/#operation/ContainerCreate Create a
    container section of the https://docs.docker.com/engine/api/v1.35/ Docker Remote API
    and the --label option to https://docs.docker.com/engine/reference/run/ docker run.
    ...
    insert_key: Optional[str] = Field(None, alias="insert-key")
...
class Ulimit(BaseModel):
    name: Name = Field(..., description="The type of the ulimit.")
...
class Options1(BaseModel):
    ...
    as the task, task definition, cluster, and container instance details to the log
    ...
    Creating a Task Definition that Uses a FireLens Configuration in the Amazon Elastic
...
class FirelensConfiguration(BaseModel):
    ...
    options: Optional[Options1] = Field(
        None,
        description=(
            ...
            " use is"
            ' "options":{"enable-ecs-log-metadata":"true|false","config-file-type:"s3|file","config-file-value":"arn:aws:s3:::mybucket/fluent.conf|filepath"}.'  # noqa: E501
            " For more information, see"
...
class ContainerDefinition(BaseModel):
    ...
    docker_labels: Optional[DockerLabels] = Field(
        None,
        alias="dockerLabels",
        description=(
